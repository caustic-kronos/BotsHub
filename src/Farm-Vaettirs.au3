...
Func SetupPlayerVaettirsFarm()
	...
	; giving more health to monk 55hp from norn title effect would screw up farm, therefore setting asura title instead for monk
	If $VaettirsPlayerProfession <> $ID_Monk Then SetDisplayedTitle($ID_Norn_Title)
	If $VaettirsPlayerProfession == $ID_Monk Then SetDisplayedTitle($ID_Asura_Title)
	Sleep(500 + GetPing())
	Return $SUCCESS
EndFunc
...
Func RunToJagaMoraine()
	...
	Local Static $pathToJaga[30][2] = [ _
		[15003.8,	-16598.1], _
		[15003.8,	-16598.1], _
		[12699.5,	-14589.8], _
		[11628,		-13867.9], _
...
Func VaettirsCheckShadowForm()
	...
	If ($VaettirsPlayerProfession <> $ID_Monk And TimerDiff($VaettirShadowFormTimer) > 20000 And GetEnergy() > 20) Or _
		...
		If $VaettirsPlayerProfession <> $ID_Monk Then UseSkillEx($Vaettir_WayOfPerfection)
	EndIf
EndFunc
...
Func VaettirsCheckObsidianFlesh()
	...
	If TimerDiff($VaettirObsidianFleshTimer) > 20000 And GetEnergy() > 30 Then ;  somehow delay is a bit longer, so not using 21500
	...
	EndIf
	If IsRecharged($Vaettir_Elementalist_StonefleshAura) Then UseSkillEx($Vaettir_Elementalist_StonefleshAura)
	If GetMapID() == $ID_Jaga_Moraine Then ; only cast energy accumulating skills when farming Vaettirs in jaga Moraine, not during run across Bjora Marches
		If TimerDiff($VaettirChannelingTimer) > 20000 And Not IsRecharged($Vaettir_Elementalist_ObsidianFlesh) And Not IsRecharged($Vaettir_Elementalist_StonefleshAura) And GetEnergy() > 5 Then
			UseSkillEx($Vaettir_Channeling)
			$VaettirChannelingTimer = TimerInit()
		EndIf
		If TimerDiff($VaettirMantraOfEarthTimer) > 40000 And GetEnergy() > 10 Then
...
Func VaettirsCheckChanneling()
	If TimerDiff($VaettirChannelingTimer) > 22000 And TimerDiff($VaettirShadowFormTimer) < 19000 Then
		UseSkillEx($Vaettir_Channeling)
		$VaettirChannelingTimer = TimerInit()
	EndIf
EndFunc
